...
```json
{
  ...
  "response_uri": "https://zamek-streliste.walletmap-club.cz/oid4vp/response",
  "nonce": "n-zamek-01",
  ...
  "eudi_rp_registration_certificate": "eyJhbGciOiJFUzI1NiIsInR5cCI6InJjLXdycCtqd3QifQ.…"
}
```

| Krok | Kdo | Co |
|------|-----|-----|
| 1 | RP | Vygeneruje efemérní šifrovací pár; veřejný klíč vloží do `client_metadata.jwks` v presentation requestu |
| 2 | Peněženka | Ověří request (WRPAC, WRPRC, claims) a z `jwks` vybere klíč dle podporovaných algoritmů |
| 3 | Peněženka | Zašifruje authorization response (`vp_token`) jako JWE; `alg` bere z vybraného JWK, `enc` z `encrypted_response_enc_values_supported` |
| 4 | RP | Dešifruje JWE **privátním klíčem** odpovídajícím `kid` z hlavičky JWE |

Účel: atributy z `vp_token` necestují v plaintextu přes front channel (prohlížeč, deeplink), i když je transport zabezpečen TLS. U běžných webových RP (`direct_post` bez `.jwt`) šifrování na aplikační vrstvě není vyžadováno.
